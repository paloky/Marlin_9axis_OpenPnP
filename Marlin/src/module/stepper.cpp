...
void Stepper::_set_position(
  LIST_N(LINEAR_AXES, const int32_t &a, const int32_t &b, const int32_t &c, const int32_t &i, const int32_t &j, \
                      const int32_t &k, const int32_t &m, const int32_t &o, const int32_t &p, const int32_t &q),  /**SG**/
  const int32_t &e
) {
  #if CORE_IS_XY
    // corexy positioning
    ...
    count_position.set(LIST_N(LINEAR_AXES, a, b, c, i, j, k, m, o, p, q));  /**SG**/
  #endif
  count_position.e = e;
}
...
void Stepper::set_position(
  ...
  planner.synchronize();
  const bool was_enabled = suspend();
  _set_position(LIST_N(LINEAR_AXES, a, b, c, i, j, k, m, o, p, q), e);  /**SG**/
  if (was_enabled) wake_up();
}
...
void Stepper::report_a_position(const xyz_long_t &pos) {
  ...
  #endif
  #if LINEAR_AXES >= 8   /**SG**/
    SERIAL_ECHOPAIR_P(SP_O_LBL, pos.o);
  #endif
  #if LINEAR_AXES >= 9   /**SG**/
    SERIAL_ECHOPAIR_P(SP_P_LBL, pos.p);
  #endif
  #if LINEAR_AXES >= 10   /**SG**/
    SERIAL_ECHOPAIR_P(SP_Q_LBL, pos.q);
  #endif
  SERIAL_EOL();
}
  ...
  void Stepper::do_babystep(const AxisEnum axis, const bool direction) {
    ...
    switch (axis) {
      ...
      case Z_AXIS: {
          ...
          const xyz_byte_t old_dir = ARRAY_N(LINEAR_AXES, X_DIR_READ(), Y_DIR_READ(), Z_DIR_READ(), I_DIR_READ(), J_DIR_READ(), \
                                                          K_DIR_READ(), M_DIR_READ(), O_DIR_READ(), P_DIR_READ(), Q_DIR_READ());  /**SG**/
