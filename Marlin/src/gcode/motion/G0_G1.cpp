  ...
  if (IsRunning()
      ...
      && !homing_needed_error(
        GANG_N(LINEAR_AXES,
          ...
          | (parser.seen(AXIS8_NAME) ? _BV(O_AXIS) : 0),    /**SG**/
          | (parser.seen(AXIS9_NAME) ? _BV(P_AXIS) : 0),    /**SG**/
          | (parser.seen(AXIS10_NAME) ? _BV(Q_AXIS) : 0))    /**SG**/
      )
    #endif
  ) {
    ...
    get_destination_from_command();                 // Get X Y Z E F (and set cutter power)
      ...
      if (MIN_AUTORETRACT <= MAX_AUTORETRACT) {
        ...
        if (fwretract.autoretract_enabled && parser.seen('E')
          && !parser.seen(GANG_N(LINEAR_AXES, "X", "Y", "Z", AXIS4_STR, AXIS5_STR, AXIS6_STR, AXIS7_STR, AXIS8_STR, AXIS9_STR, AXIS10_STR)   /**SG**/
        ) {
